    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error('Nenhum agente configurado ainda — termine a Configuração primeiro.');
        } else {
    ...
    <div className="flex h-[60vh] min-h-[420px] flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">Laboratório</span>
          <span className="text-xs text-muted-foreground">
            — teste respostas como se fosse um cliente
          </span>
        </div>
        <Button
          variant="ghost"
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1 text-xs">
              ...
              bot de resposta automática — incluindo transferência para humano.
            </p>
            {onGoToSetup && (
              <Button
                variant="link"
              ...
              >
                Ainda não está configurado? Vá para Configuração <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
            ...
            >
              ...
              {t.role === 'assistant' && t.handoff && (
                ...
                >
                  ...
                  Encaminharia para um humano aqui
                </p>
              )}
            </div>
            {t.role === 'user' && (
