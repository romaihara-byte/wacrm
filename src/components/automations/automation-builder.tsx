...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  ...
  { value: "keyword_match", label: "Palavra-chave encontrada", hint: "A mensagem contém palavra(s)-chave específica(s)" },
  ...
  { value: "time_based", label: "Baseado em tempo", hint: "Em uma agenda recorrente" },
]
  ...
  return (
    <>
      <FieldBlock label="Pipeline">
        ...
        >
          ...
          {pipelineId && !selectedPipeline && (
            <option value={pipelineId}>{pipelineId} (pipeline desconhecido)</option>
          )}
        </select>
      </FieldBlock>
      <FieldBlock label="Etapa">
        ...
        >
          <option value="">
            {pipelineId ? "Selecione uma etapa..." : "Selecione um pipeline primeiro..."}
          </option>
          {stageOptions.map((s) => (
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="ex.: preço, solicitar demonstração, falar com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
...
function StepEditor({
  ...
  switch (step.step_type) {
      ...
      return (
        <>
          <FieldBlock label="Modo">
            ...
            >
              <option value="round_robin">Rodízio automático</option>
    ...
    case "condition":
      return (
        <>
          <FieldBlock label="Condição">
            <select
              value={(cfg.subject as string) ?? "tag_presence"}
              onChange={(e) => set({ subject: e.target.value })}
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="tag_presence">Etiqueta presente</option>
    ...
    case "close_conversation":
      return (
        <p className="text-xs text-muted-foreground">
          Define o status da conversa como &quot;closed&quot;. Nenhuma configuração necessária.
        </p>
      )
    default:
      return null
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "wait":
      return formatWaitDuration(step.step_config.amount, step.step_config.unit)
    case "condition":
      return `quando ${friendlyConditionSubject(step.step_config.subject)}`
    case "send_webhook":
...
function formatWaitDuration(amountRaw: unknown, unitRaw: unknown): string {
  const amount = typeof amountRaw === "number" ? amountRaw : Number(amountRaw)
  const safeAmount = Number.isFinite(amount) && amount > 0 ? amount : 1
  const unit = String(unitRaw ?? "")

  if (unit === "days") return `${safeAmount} ${safeAmount === 1 ? "dia" : "dias"}`
  if (unit === "hours") return `${safeAmount} ${safeAmount === 1 ? "hora" : "horas"}`
  if (unit === "minutes") {
    return `${safeAmount} ${safeAmount === 1 ? "minuto" : "minutos"}`
  }
  return `${safeAmount}`
}

function friendlyConditionSubject(subjectRaw: unknown): string {
  const subject = String(subjectRaw ?? "?")
  if (subject === "time_of_day") {
    return "o horário corresponder à condição configurada"
  }
  if (subject === "tag_presence") return "houver etiqueta correspondente"
  if (subject === "contact_field") return "o campo do contato corresponder à condição"
  if (subject === "message_content") {
    return "o conteúdo da mensagem corresponder à condição"
  }
  return subject
}
